namespace Eisdiele {
    ...
    function spawnCustomer(): void {
        const newCustomer = new Kunde({ x: crc2.canvas.width, y: crc2.canvas.height / 2 }, { x: -2, y: 0 }, crc2.canvas.width, crc2.canvas.height);
        allCustomers.push(newCustomer);
    }

    function updateCustomers(): void {
    ...
    function isCustomerClicked(customer: Kunde, x: number, y: number): boolean {
