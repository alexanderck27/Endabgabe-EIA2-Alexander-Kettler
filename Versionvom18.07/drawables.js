...
(function (Eisdiele) {
    ...
    function drawIceCreamParlor(crc2) {
        crc2.save(); // Speichert den aktuellen Zustand des Canvas
    ...
    document.addEventListener("DOMContentLoaded", () => {
        const canvas = document.getElementById("myCanvas"); // Canvas-Element abrufen
        const crc2 = canvas.getContext("2d"); // 2D-Kontext des Canvas-Elements abrufen
        drawScene(crc2); // Szene zeichnen
    });
    const allObjects = []; // Erstellen eines Arrays für alle zu zeichnenden Objekte
    // Funktion zum Zeichnen der Szene
    function drawScene(crc2) {
